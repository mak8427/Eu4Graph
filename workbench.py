...
print('type the skanderbeg save code')
save = '682bfb'
print('do you want compare this save with an older one?[y/n]')

save2 ='b2f19e'

print('do you want to print only players?[y/n]')
ia = 'n'
try:
...
tags = df2['tag'].tolist()
players = df2['countryName'].tolist()
i = 0
...
def human(df):
    Humanplayers = df.loc[(df['tag'] == 'TUR') | (df['tag'] == 'FRA') | (df['tag'] == 'MOS') | (df['tag'] == 'CAS') | (
...
def prova(listadf):
    ...
    for df in listadf:
        dict = sostituisci(df)
        humandev = human(df[['tag', 'totaldevelopment']])
...
k = 0
# colori grafico
colori=['black','grey','rosybrown','brown','darkred','red','tomato','coral','orangered','sienna','peru','orange','goldenrod','gold','khaki','olive','yellow','lawngreen','green','lime','teal','cyan','steelblue','navy','blue','blueviolet','indigo','violet','magenta','deeppink']
...
for nation in listaz:
    if count == len(colori)-1:
        count=0
    colornazioni[nation]=colori[count]
    count=count+1
...
for s in ogni:
    if k == 0:  # dev
        ...
        for i, v in enumerate(s.loc[len(s) - 1]):  # testo su barre
            ax1.text(i - len(str(round(v,1)))*sz, v + 0.5, str(round(v, 1)), color='black', fontweight='bold',size=sizes)
    ...
    if k == 1:  # income
        ...
        for i, v in enumerate(percentuali):  # testo su linea
            ax2.text(i - 0.10, v + 1, str(round(v)) + '%', color='black', fontweight='bold',size=sizes)
        for i, v in enumerate(s.loc[len(s) - 1]):  # testo su barre
            ax1.text(i - len(str(round(v)))*sz, v + 0.5, str(round(v)), color='black', fontweight='bold',size=sizes)
    ...
    if k == 2:  # valore immobili
        ...
        s = s.iloc[:, range(0, y)]
        ax2 = s.loc[len(s) - 1].plot.pie(autopct=lambda pct: func(pct, s.loc[len(s) - 1]), startangle=90, radius=1.3,
                                         )
    ...
    if k == 4:
        ...
        for i, v in enumerate(s.loc[len(s) - 1]):  # testo su barre
            if v>=100000:
               ax1.text(i - len(str(round(v/1000000,1))+'M')*sz, v + 0.5, str(round(v / 1000000, 1)) + 'M', color='black', fontweight='bold',size=sizes)
            else:
               ax1.text(i - len(str(round(v/1000,1))+'k')*sz, v + 0.5, str(round(v / 1000, 1)) + 'k', color='black', fontweight='bold',size=sizes)
    ...
    if k == 13:
        try:
            ...
            s = s.sort_values(by=(len(s) - 1), axis=1, ascending=False)  # sort data
            s = s.iloc[:, range(0, y)]
            print('clickssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss')
            print(s)
            coloriar = []
            for nomej in s.columns:
                coloriar.append(colornazioni[nomej])
...
for i, v in enumerate(dfavgdev.loc[len(dfavgdev) - 1]):  # testo su barre
    ax1.text(i - len(str(round(v, 1)))*sz, v + 0.005, str(round(v, 1)), color='black', fontweight='bold',size=sizes)
plt.title('avg dev')
...
for i, v in enumerate(dfavgmanpower.loc[len(dfavgmanpower) - 1]):  # testo su barre
    ax1.text(i - len(str(round(v / 1000, 1))+'k')*sz, v + 0.05, str(round(v / 1000, 1)) + 'k', color='black', fontweight='bold',size=sizes)
...
def differenza(playerincome, cosa):
    ...
    diffincome = []
    diffincomeper = []
    diff = lambda x: round(x.iloc[1] - x.iloc[0], 2)
    diffper = lambda x: round(((x.iloc[1] / x.iloc[0]) * 100) - 100, 1)
    ...
    for i, v in enumerate(dfdiffincome.iloc[0]):  # testo su linea
        if v > 0:
            ax2.text(i - len(str(round(v))+'+%')*sz, 0.5, '+' + str(round(v)) + '%', color='black', fontweight='bold',size=sizes)
        else:
            try:
    ...
    for i, v in enumerate(dfdiffincome.iloc[1]):  # testo su barre
        if v > 1000:
            ax1.text(i - len(str(int(v)))*sz, v + 0.1, str(round(v / 1000)) + 'k', color='black', fontweight='bold',size=sizes)
        else:
            ax1.text(i - len(str(int(v)))*sz, v + 0.1, str(round(v)), color='black', fontweight='bold',size=sizes)
