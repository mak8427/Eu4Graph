...
for s in ogni:
    ...
    if k == 2:  # valore immobili
        ...
        ax2 = s.loc[len(s) - 1].plot.pie(autopct=lambda pct: func(pct, s.loc[len(s) - 1]), startangle=90, radius=1.3,colors=coloriar)
        plt.grid(axis='y',alpha=alphagrid)
    ...
    if k == 7:
        ...
        for i, v in enumerate(s.loc[len(s) - 1]):  # testo su barre
            ax1.text(i - len(str(round(v / 1000, 1)) + 'k') * sz, v + 0.5, str(round(v / 1000, 1)) + 'k', color='black',
...
def differenza(playerincome, cosa):
    ...
    for i, v in enumerate(dfdiffincome.iloc[1]):  # testo su barre
        if v > 1000:
            ax1.text(i - len(str(round(v / 1000,2))+'k') * sz, v + 0.1, str(round(v / 1000)) + 'k', color='black', fontweight='bold',
                     size=sizes)
        elif v<-1000:
            ax1.text(i - len(str(round(v / 1000,2))+'k') * sz, v + 0.1, str(round(v / 1000)) + 'k', color='black', fontweight='bold',size=sizes)
        else:
